...
import static com.taint.tainting.Log.print;
...
public class XTaintingAnnotatedTypeFactory extends AccumulationAnnotatedTypeFactory {
  ...
  public XTaintingAnnotatedTypeFactory(BaseTypeChecker checker) {
    super(checker, RPossiblyValidated.class, RUntainted.class, null);
    enableLibraryCheck = checker.getBooleanOption(XTaintingChecker.ENABLE_LIBRARY_CHECKER, true);
    enablePolyTaintInference =
        checker.getBooleanOption(XTaintingChecker.ENABLE_POLY_TAINT_INFERENCE, true);
    enableTypeArgumentInference =
        checker.getBooleanOption(XTaintingChecker.ENABLE_TYPE_ARGUMENT_INFERENCE, true);
    enableValidationCheck = checker.hasOption(XTaintingChecker.ENABLE_VALIDATION_CHECKER);
    enableSideEffect = checker.hasOption(XTaintingChecker.ENABLE_SIDE_EFFECT);
    String givenAnnotatedPackages = checker.getOption(XTaintingChecker.ANNOTATED_PACKAGES);
    // make sure that annotated package names are always provided and issue error otherwise
    if (givenAnnotatedPackages == null) {
      if (checker.hasOption(XTaintingChecker.ANNOTATED_PACKAGES)) {
        throw new UserError(
  ...
  protected TreeAnnotator createTreeAnnotator() {
    ...
    return new ListTreeAnnotator(
        super.createTreeAnnotator(), new XTaintingTreeAnnotator(this, handler, context));
  }
  ...
  protected QualifierHierarchy createQualifierHierarchy() {
    return new XTaintingQualifierHierarchy(this.getSupportedTypeQualifiers(), this.elements);
  }

  @Override
  protected DefaultForTypeAnnotator createDefaultForTypeAnnotator() {
    return new XTypeAnnotator(this);
  }
  ...
  private class XTaintingQualifierHierarchy extends AccumulationQualifierHierarchy {
     ...
     */
    protected XTaintingQualifierHierarchy(
